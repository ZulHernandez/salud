...
const CoViewer = ({ drink, kind }) => {
	...
	return (
		<div
			className={`${kind}-viewer`}
			onMouseOver={() => setHover(true)}
			onMouseOut={() => setHover(false)}
		>
			{view !== "cuadricula" ? (
				<div style={{ width: "0px", height: "0px" }}>
					<p className={`${kind}-sticker`}>{t("viewer-sticker")}</p>
				</div>
			) : null}
			<div style={{ height: "0px" }}>
				<div
					...
					style={{
						...
						backgroundColor:
							drink.mix_color || drink.ingredients[0].alcohol.color,
					}}
				>
					{(() => {
						let totalQuantity;
			...
			{drink.ganish.map((ganish, index) => (
				<div key={index} style={{ height: "0px" }}>
					<div
						className="card-viewer-ganish"
						style={{
							mixBlendMode:
								ganish.name === "ice cube" ? "soft-light" : "multiply",
						}}
					>
						<img
							style={{
